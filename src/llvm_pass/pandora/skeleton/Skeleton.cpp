...
namespace {
  struct SkeletonPass : public FunctionPass {
    ...
    virtual bool runOnFunction(Function &F) {
      ...
      Constant *slpFunc1 = F.getParent()->getOrInsertFunction("slp1", uslpType);
      Constant *slpFunc2 = F.getParent()->getOrInsertFunction("slp2", uslpType);
      Constant *slpFunc3 = F.getParent()->getOrInsertFunction("slp3", uslpType);
        ...
        if (auto *op = dyn_cast<BinaryOperator>(it)) {
          ...
          if (op->getOpcode() == Instruction::Xor)
          {
            ...
            if (exist_key(ci2->getSExtValue())){
              ...
              builder.CreateCall(slpFunc1, args);
              builder.CreateCall(slpFunc2, args);
              //builder.CreateCall(slpFunc3, args);
            }
          }
        }
      }
